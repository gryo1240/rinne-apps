 ...
 * 「東証REIT指数（配当込みを含む）に連動する内国ETF」のうち、2026-08-25時点でJPXの一覧から
 * 抽出できた12銘柄を収録する（それ以降の新規上場・償還は追跡していない）。
 * ★記事・告知文に「全12本」「12本すべて」と網羅を断定して書かないこと（2026-09-07 オーナー指摘）。
 *   一覧を1回見たことは「これで全部」の証明にならず、時間が経つと勝手に偽になる。
 *   記事側の言い回しは「〜の中から12本を収録」に統一する。
 ...
 *  4. 収録数が変わったら、下の「12」を手で書いている全箇所を同時に直す。
 *     機械で守られているのはこの2つだけ（test.js が FUNDS.length と LEDGER の件数一致を検査する）:
 *       - このファイルの FUNDS 配列 / test.js の LEDGER
 *     残りは人が書いた文章なので、grep しないと取り残す（2026-09-07に洗い出した実在箇所）:
 *       - index.html … 「収録している12銘柄の決算日は」「全12銘柄を確認できます」の2箇所
 *       - logic.js のコメント … 「収録12銘柄」「12銘柄あると」等
 *       - .company/media/rinne-blog/jreit-settlement-calendar.md（決算日データの正本）
 *       - 記事 post=11566 の本文と、その正本
 *         .company/media/rinne-blog/articles/2026-08-25-jreit-calendar/page.html
 *     ★記事側は「〜の中から12本を収録」の形を崩さないこと（上の注意書きを参照）
 */
