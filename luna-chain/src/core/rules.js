...
import { N, DEF_W, DEF_H, T_CLOUD, T_NORMAL, T_STARDUST, buildGeometry, orthOf } from './board.js';

/* ══════════════════════════════════════════════════════════
   ★手番の上限は、盤のマス数に比例させる★（2026-09-10 オーナー報告を受けて修正）
     > 盤面を大きくして遊んだ時に、途中の段階で勝敗がついて負けになったときがある

   ★何が起きていたか★
     上限は 120手の**固定値**だった。この数字は盤が 6×7（42マス）に固定だった時代に
     「全滅決着まで平均75〜90手番かかる」という実測から決めたもの（仕様書§0-1の表3）。
     2026-09-09 に盤の大きさを可変（4×5〜8×10）にしたとき、**ここを見直さなかった**。
     実測（1条件120局）:
       6×7 (42マス)  上限打切り  0.0〜1.7%
       8×8 (64マス)  上限打切り 21.7〜45.0%  ←打切り時点で盤は88〜90%しか埋まっていない
     つまり大きい盤では、勝負がつく前に打ち切られて光の数で判定されていた。
     しかも打ち切られたことが画面に出ないので、理由なく「まけ」が出たように見える。

   ★なぜ上限そのものは残すのか★
     無限に続かないことを保証するため（停止性の議論は board.js 冒頭）。
     比例させても、その目的は変わらず達成される。

   ★なぜ「置ける手が無くなるまで」にしないのか★
     legalMoves は「空きマス or 自分のマス」を全部返すので、ほぼ空にならない。
     終了条件として機能しない。

   ★6×7 は必ず 120 のままにすること★
     ここが1でもズレると、棋譜・勝率・段位の記録がすべて動く。テストで固定してある。
   ══════════════════════════════════════════════════════════ */
export const DEF_CELLS = DEF_W * DEF_H;      // 42
export const TURNS_PER_CELL = 120 / DEF_CELLS;
/** そのマス数なら手番の上限はいくつか */
export const maxTurnsFor = (cells = N) => Math.round(TURNS_PER_CELL * Math.max(1, cells));
...
export function newGame(opts = {}) {
  ...
  const s = {
    ...
    /* 手番（片方の1手）の上限。★盤のマス数に比例★（6×7=42マスで120手＝もとの値）
       明示指定は必ず優先する（テストとシミュレーターが依存している） */
    maxTurns: opts.maxTurns ?? maxTurnsFor(N),
    winner: 0,
    endReason: '',
