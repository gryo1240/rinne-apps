 ...
 * 5. 国民年金保険料月額: 日本年金機構
 * 6. 税制（基礎控除の特例・給与所得控除）: 令和8年度改正の特例（基礎控除104万・給与所得控除74万）は
 *    令和8・9年分の措置。令和10年分以後は本則（基礎控除62万・給与所得控除は収入×30%+8万,下限69万）に戻り、
 *    以後2年ごとに全国消費者物価指数の変化率で見直される。→ 2027年末に要更新
 * 7. 制度施行日: 賃金要件(月8.8万)撤廃=2026-10-01(予定・政令待ち)。
 *    企業規模要件の段階撤廃: 2027-10(36-50人)→2029-10(21-35人)→2032-10(11-20人)→2035-10(全事業所)
 */

var CONFIG = {
  updated: "2026-08-25", // 画面に自動表示される「制度・料率の確認日」
  ...
  // ---- 税（令和8年度税制改正後・2026年(令和8年)分以後） ----
  // 出典: 国税庁「令和8年4月 源泉所得税の改正のあらまし」(1)(2)
  //   https://www.nta.go.jp/publication/pamph/gensen/2026kaisei.pdf
  //   令和8年12月1日施行・令和8年分以後の所得税に適用
  // 住民税は名古屋市「令和9年度以降適用される市民税・県民税に関する主な税制改正」で確認
  //   (令和8年中の収入 → 令和9年度分住民税に適用)
  kyuyoKoujo: 740000,        // 給与所得控除の最低保障額（65万→74万・給与収入220万円以下）
  shotokuzeiKiso: 1040000,   // 所得税の基礎控除（本則62万＋特例加算42万。合計所得489万円以下・令和8/9年分）
  juuminKiso: 430000,        // 住民税の基礎控除（令和8年度改正でも43万円のまま据置）
  juuminHikazeiKyuyo: 1190000, // 住民税非課税となる給与収入ライン（1級地・単身。合計所得45万円以下＝45万+給与所得控除74万）
  shotokuzeiRate: 0.05105,   // 所得税率5%×復興特別所得税1.021
  juuminShotokuwariRate: 0.10,
  juuminChousei: 2500,       // 調整控除（合計所得200万以下・基礎控除分＝人的控除の差5万円×5%）
                             // 令和8年度改正でも住民税の基礎控除43万円・人的控除の差5万円とも変更なし（2026-08-25確認）
...
function buildCurve(p, maxHours) {
  ...
  // 住民税非課税ライン（119万円）でも均等割の段差が出るので、境界点として明示的に打つ
  boundaries.push(CONFIG.juuminHikazeiKyuyo / 52 / p.hourly);
